...
class AuGraphEnv(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.action_space = Box(low=np.zeros(5), high=np.ones(5), dtype=np.float32)  # 5个连续动作空间，0-255，以5增长
    ...
    def step(self, action) -> tuple:
        ...
        if request_index_current == 0:
            ...
            flag, wave_used, au_edge_collection = RWA.route_wave_assign(action_t,request_index_current)  # flag表示是否选路成功，wave_used表示使用的波长（用于计算奖励）
        else:
        ...
        if flag:
            ...
            print('id', request_index_current, 'weight', action_t, "wave_last", AuGraphEnv.wavelength_last, "wave_cur",
                  wave_used, "reward", reward, )

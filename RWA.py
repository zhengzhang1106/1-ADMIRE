...
def route_wave_assign(weight, index):
    ...
    if route_tmp:
        ...
        wave_used = Compute.compute_wavelength(Database.links_physical)  # 部署完当前业务后，所有使用的波长数量
        virtual_hop_num = Compute.compute_hop(route_tmp)  # 虚拟拓扑跳数
        return True, wave_used, au_edge_collection
    else:

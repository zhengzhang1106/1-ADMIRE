...
# path1 = Database.data_path21
# path1 = 'noise/noise1.csv'
# path1 = Database.data_path_random_list[6]
# path1 = Database.data_path_list[21]
# path2 = path1
# print("path1",path1)
# print("path2",path2)
...
service_traffic_all1 = pd.read_csv(path1, engine='python',
                                   usecols=list(range(3, Database.time + 3)))  # 记录所有业务的流量（多个时刻）
# service_traffic_all = pd.read_excel(data_path,sheet_name='0822',usecols=list(range(3,Database.time+3)))     # 记录所有业务的流量（多个时刻）
service_traffic_all1 = service_traffic_all1.values.tolist()

# service_traffic_all2 = pd.read_csv(path2, engine='python',
#                                    usecols=list(range(3, Database.time + 3)))  # 记录所有业务的流量（多个时刻）
# service_traffic_all2 = service_traffic_all2.values.tolist()
